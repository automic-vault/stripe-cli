...
func (p *Profile) HasAPIKey(livemode bool) bool {
	...
	if keyring.ProtectsAllAPIKeys() {
		...
		return isRedactedAPIKey(p.ReadProfileString(field))
	}
...
func (p *Profile) writeProfile(runtimeViper *viper.Viper) error {
	...
	if p.LiveModeAPIKey != "" {
		...
		runtimeViper.Set(p.configFieldForWrite(LiveModeAPIKeyName), redactedAPIKeyValue(strings.TrimSpace(p.LiveModeAPIKey)))
	...
	if p.TestModeAPIKey != "" {
		...
		runtimeViper.Set(p.configFieldForWrite(TestModeAPIKeyName), testModeAPIKey)
...
func (p *Profile) credentialKey(field string) string {
	...
	if accountID == "" {
		accountID = strings.TrimSpace(p.ReadProfileString(AccountIDName))
	}
	if accountID == "" {
		return p.GetConfigField(field)
	}
